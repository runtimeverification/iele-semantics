            ...
            stage('Build Image') {
              ...
              steps {
                ...
                sh '''
                  ...
                  docker image build . --file package/docker/Dockerfile --tag "${DOCKERHUB_REPO}:${BIONIC_COMMIT_TAG}" --build-arg K_COMMIT=$(cat deps/k_release | cut --characters=2-) --build-arg DISTRO='bionic'
                  docker image build . --file package/docker/Dockerfile --tag "${DOCKERHUB_REPO}:${FOCAL_COMMIT_TAG}"  --build-arg K_COMMIT=$(cat deps/k_release | cut --characters=2-) --build-arg DISTRO='focal'
